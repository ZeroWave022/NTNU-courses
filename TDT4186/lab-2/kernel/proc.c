...
uint64 time_slices[MLFQ_QUEUES_NUM] = MLFQ_TIME_SLICES;
uint64 ticks_since_boost = 0;
...
void yield(uint64 reason)
{
    ...
    ticks_since_boost += ticks - p->started_at;
    p->runtime += ticks - p->started_at;

    // Did the process use up its time slice?
    if (p->runtime >= time_slices[p->priority])
    {
        // Decrease priority if possible
        if (p->priority < MLFQ_QUEUES_NUM - 1)
        {
            p->priority++;
            p->runtime = 0;
        }
    ...
    // Should all processes be boosted to the top-most queue?
    if (ticks_since_boost >= MLFQ_PRIORITY_BOOST_INTERVAL)
    {
